...
-- ============================================
CREATE OR REPLACE FUNCTION dw.carregar_fato_contas_financeiras(p_cliente_id UUID DEFAULT NULL)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    v_data_id INTEGER;
BEGIN
    -- Limpar fatos existentes para o cliente (se especificado) ou todos
    IF p_cliente_id IS NOT NULL THEN
        DELETE FROM dw.fato_contas_financeiras WHERE cliente_id = p_cliente_id;
    ELSE
    ...
    FROM contas_pagar_detalhadas cpd
    LEFT JOIN dw.dim_calendario dc ON dc.data = cpd.data_vencimento
    LEFT JOIN dw.dim_categoria dc_cat ON dc_cat.cliente_id = cpd.cliente_id 
                                     AND dc_cat.categoria_api_id = cpd.categoria_id
    LEFT JOIN (
        SELECT DISTINCT ON (cliente_id, categoria_financeira_id) 
            cliente_id,
            categoria_financeira_id,
            categoria_dre_id
        FROM dw.dim_categoria_dre
        WHERE categoria_financeira_id IS NOT NULL
    ) dc_cat_dre ON dc_cat_dre.cliente_id = cpd.cliente_id
                AND dc_cat_dre.categoria_financeira_id = dc_cat.categoria_api_id -- Relação através de categoria financeira
    LEFT JOIN dw.dim_centro_custo dc_cc ON dc_cc.cliente_id = cpd.cliente_id 
                                       AND dc_cc.centro_custo_api_id = cpd.centro_custo_id
    LEFT JOIN dw.dim_pessoa dc_pessoa ON dc_pessoa.cliente_id = cpd.cliente_id 
                                     AND dc_pessoa.pessoa_api_id = cpd.fornecedor_id
    WHERE (p_cliente_id IS NULL OR cpd.cliente_id = p_cliente_id)
    ...
    SELECT 
        ...
        NULL::TIMESTAMP WITH TIME ZONE AS data_criacao,
        NULL::TIMESTAMP WITH TIME ZONE AS data_alteracao
    FROM contas_receber_detalhadas crd
    LEFT JOIN dw.dim_calendario dc ON dc.data = crd.data_vencimento
    LEFT JOIN dw.dim_categoria dc_cat ON dc_cat.cliente_id = crd.cliente_id 
    ...
    LEFT JOIN (
        ...
        FROM dw.dim_categoria_dre
        WHERE categoria_financeira_id IS NOT NULL
    ) dc_cat_dre ON dc_cat_dre.cliente_id = crd.cliente_id
                AND dc_cat_dre.categoria_financeira_id = dc_cat.categoria_api_id -- Relação através de categoria financeira
    LEFT JOIN dw.dim_centro_custo dc_cc ON dc_cc.cliente_id = crd.cliente_id 
                                       AND dc_cc.centro_custo_api_id = crd.centro_custo_id
    LEFT JOIN dw.dim_pessoa dc_pessoa ON dc_pessoa.cliente_id = crd.cliente_id 
                                     AND dc_pessoa.pessoa_api_id = crd.cliente_conta_id
    WHERE (p_cliente_id IS NULL OR crd.cliente_id = p_cliente_id)
    ON CONFLICT (cliente_id, conta_id, parcela_id, categoria_id, centro_custo_id)
    DO UPDATE SET
...
COMMENT ON FUNCTION dw.carregar_fato_contas_financeiras(UUID) IS 'Carrega fato unificado de contas a pagar e receber. Se p_cliente_id for NULL, carrega todos os clientes.';

-- ============================================
-- FUNÇÃO: carregar_fato_vendas
-- Carrega fato de vendas a partir de vendas_detalhadas
-- ============================================
CREATE OR REPLACE FUNCTION dw.carregar_fato_vendas(p_cliente_id UUID DEFAULT NULL)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    v_log_entry := jsonb_build_object(
        ...
        'data', jsonb_build_object('p_cliente_id', p_cliente_id),
    ...
    -- Limpar fatos existentes para o cliente (se especificado) ou todos
    IF p_cliente_id IS NOT NULL THEN
        DELETE FROM dw.fato_vendas WHERE cliente_id = p_cliente_id;
    ELSE
    ...
    WHERE (p_cliente_id IS NULL OR vd.cliente_id = p_cliente_id);
    ...
    FROM vendas_detalhadas vd
    LEFT JOIN vendas v ON vd.venda_id = v.venda_id AND vd.cliente_id = v.cliente_id
    -- #region agent log
    -- Log antes do JOIN para verificar dados de entrada
    -- #endregion
    LEFT JOIN dw.dim_calendario dc ON dc.data = vd.data
    LEFT JOIN dw.dim_vendedor dv ON dv.cliente_id = vd.cliente_id 
                                 AND dv.vendedor_api_id = vd.vendedor_id
    LEFT JOIN dw.dim_pessoa dp ON dp.cliente_id = vd.cliente_id 
                               AND (dp.pessoa_api_id = vd.cliente_venda_id OR dp.pessoa_api_id = vd.cliente_uuid)
    LEFT JOIN dw.dim_categoria dc_cat ON dc_cat.cliente_id = vd.cliente_id 
                                      AND dc_cat.categoria_api_id = vd.venda_id_categoria
    LEFT JOIN dw.dim_centro_custo dc_cc ON dc_cc.cliente_id = vd.cliente_id 
                                        AND dc_cc.centro_custo_api_id = vd.venda_id_centro_custo
    LEFT JOIN dw.dim_conta_financeira dc_cf ON dc_cf.cliente_id = vd.cliente_id 
    ...
    WHERE (p_cliente_id IS NULL OR fv.cliente_id = p_cliente_id)
    AND fv.tipo IS NULL;
...
COMMENT ON FUNCTION dw.carregar_fato_vendas(UUID) IS 'Carrega fato de vendas a partir de vendas_detalhadas com todos os campos expandidos. Se p_cliente_id for NULL, carrega todos os clientes.';

-- ============================================
-- FUNÇÃO: carregar_fato_vendas_itens
-- Carrega fato de itens de vendas
-- ============================================
CREATE OR REPLACE FUNCTION dw.carregar_fato_vendas_itens(p_cliente_id UUID DEFAULT NULL)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    -- Limpar fatos existentes para o cliente (se especificado) ou todos
    IF p_cliente_id IS NOT NULL THEN
        DELETE FROM dw.fato_vendas_itens WHERE cliente_id = p_cliente_id;
    ELSE
    ...
    INNER JOIN vendas v ON v.cliente_id = vi.cliente_id AND v.venda_id = vi.venda_id
    LEFT JOIN dw.dim_calendario dc ON dc.data = v.data
    WHERE (p_cliente_id IS NULL OR vi.cliente_id = p_cliente_id)
...
COMMENT ON FUNCTION dw.carregar_fato_vendas_itens(UUID) IS 'Carrega fato de itens de vendas. Se p_cliente_id for NULL, carrega todos os clientes.';

-- ============================================
-- FUNÇÃO: carregar_fato_contratos
-- Carrega fato de contratos
-- ============================================
CREATE OR REPLACE FUNCTION dw.carregar_fato_contratos(p_cliente_id UUID DEFAULT NULL)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    -- Limpar fatos existentes para o cliente (se especificado) ou todos
    IF p_cliente_id IS NOT NULL THEN
        DELETE FROM dw.fato_contratos WHERE cliente_id = p_cliente_id;
    ELSE
    ...
    LEFT JOIN dw.dim_pessoa dp ON dp.cliente_id = c.cliente_id 
...
COMMENT ON FUNCTION dw.carregar_fato_contratos(UUID) IS 'Carrega fato de contratos. Se p_cliente_id for NULL, carrega todos os clientes.';

-- ============================================
-- FUNÇÃO: carregar_fato_saldos_contas
-- Carrega fato de saldos de contas financeiras (histórico temporal)
-- ============================================
CREATE OR REPLACE FUNCTION dw.carregar_fato_saldos_contas(p_cliente_id UUID DEFAULT NULL)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    -- Limpar fatos existentes para o cliente (se especificado) ou todos
    IF p_cliente_id IS NOT NULL THEN
        DELETE FROM dw.fato_saldos_contas WHERE cliente_id = p_cliente_id;
    ELSE
    ...
    SELECT 
        ...
        sc.saldo_atual,
        sc.data_coleta
    FROM saldos_contas sc
    LEFT JOIN dw.dim_calendario dc ON dc.data = DATE(sc.data_coleta)
    LEFT JOIN dw.dim_conta_financeira dcf ON dcf.cliente_id = sc.cliente_id 
    ...
    v_log_entry := jsonb_build_object(
        ...
        'data', jsonb_build_object('row_count', v_registros, 'p_cliente_id', p_cliente_id),
        'timestamp', extract(epoch from now())::bigint * 1000
    );
    PERFORM dw.debug_log(v_log_entry);
    -- #endregion
...
COMMENT ON FUNCTION dw.carregar_fato_saldos_contas(UUID) IS 'Carrega fato de saldos de contas financeiras. Tabela de histórico temporal - permite múltiplos registros por conta. Se p_cliente_id for NULL, carrega todos os clientes.';
...
-- ============================================
CREATE OR REPLACE FUNCTION dw.carregar_dw_completo(p_cliente_id UUID DEFAULT NULL)
RETURNS JSON
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    -- ETAPA 1: Carregar dimensão calendário (uma vez, não precisa de cliente_id)
    BEGIN
    ...
    BEGIN
        SELECT dw.carregar_dim_categoria(p_cliente_id) INTO v_dim_categoria;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_dim_categoria_dre(p_cliente_id) INTO v_dim_categoria_dre;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_dim_centro_custo(p_cliente_id) INTO v_dim_centro_custo;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_dim_pessoa(p_cliente_id) INTO v_dim_pessoa;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_dim_conta_financeira(p_cliente_id) INTO v_dim_conta_financeira;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_dim_vendedor(p_cliente_id) INTO v_dim_vendedor;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_fato_contas_financeiras(p_cliente_id) INTO v_fato_contas_financeiras;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_fato_vendas(p_cliente_id) INTO v_fato_vendas;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_fato_vendas_itens(p_cliente_id) INTO v_fato_vendas_itens;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_fato_contratos(p_cliente_id) INTO v_fato_contratos;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
    ...
    BEGIN
        SELECT dw.carregar_fato_saldos_contas(p_cliente_id) INTO v_fato_saldos_contas;
        v_resultado := jsonb_set(
            v_resultado::jsonb,
            '{etapas}',
            (v_resultado->'etapas')::jsonb || jsonb_build_array(jsonb_build_object(
...
COMMENT ON FUNCTION dw.carregar_dw_completo(UUID) IS 'Executa todo o processo ETL do Data Warehouse em ordem: dimensões primeiro, depois fatos. Retorna JSON com estatísticas de cada etapa. Se p_cliente_id for NULL, processa todos os clientes.';
